...
def build_jobs(args) -> list[Job]:
    from training.train_ppo import DATASET_PROFILES

    data = Path(args.data)
    model_dir = Path(args.output)
    profile_name = getattr(args, "dataset_profile", "klines") or "klines"
    if profile_name not in DATASET_PROFILES:
        raise SystemExit(f"unknown --dataset-profile {profile_name!r}")
    profile = DATASET_PROFILES[profile_name]

    if data.is_file() and data.suffix == ".parquet":
        labeled = data
        holdout = (
            Path(args.holdout) if args.holdout else data.parent / Path(profile["holdout"]).name
        )
    else:
        labeled = data / Path(profile["data"]).name
        holdout = Path(args.holdout) if args.holdout else data / Path(profile["holdout"]).name
    ...
    for name in requested:
        if name in ("trend", "mean_reversion", "scalper"):
            out = model_dir / "agents" / f"{name}_{profile_name}_ppo.zip"
            cmd = [
                PYEXE, "-m", "training.train_ppo",
                "--agent", name,
            ...
            out = model_dir / "regime" / f"hmm_4state_{profile_name}.pkl"
            cmd = [
                PYEXE, "-m", "training.fit_regime",
                "--data", str(labeled),
                "--output", str(out),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default="klines",
        choices=["klines", "hf"],
        help="klines=swing labeled set; hf=scalp HF labeled set",
    )
