...
# Dataset profile presets — selects which labeled dataset + reward wiring to use.
DATASET_PROFILES: dict[str, dict] = {
    "klines": {
        "data": "./datasets/processed/quant_labeled.parquet",
        "holdout": "./datasets/processed/quant_holdout.parquet",
        "description": "15m/1h/4h/1d Binance klines + macro + TradFi (swing track)",
        # swing_1h labels — no 1m/5m columns exist on this track
        "realized_fallback": (
            "forward_return_1h",
            "forward_return_4h",
            "forward_return_1d",
    ...
    "hf": {
        "data": "./datasets/processed/quant_labeled_hf.parquet",
        "holdout": "./datasets/processed/quant_holdout_hf.parquet",
        "description": "HF VPS tick/depth microstructure (scalp track)",
        "realized_fallback": (
            "forward_return_1m",
            "forward_return_5m",
            "forward_return_15m",
...
# Agent preferred target → profile-aware fallbacks when column absent.
_AGENT_REALIZED_PREFERENCE: dict[str, tuple[str, ...]] = {
    "trend": (
        "forward_return_5m",
        "forward_return_15m",
        "forward_return_4h",
...
def _resolve_realized_col(df, agent: str, dataset_profile: str | None = None) -> str:
    """Pick a forward-return column that actually exists on this labeled frame."""
    preferred: list[str] = []
    preferred.extend(_AGENT_REALIZED_PREFERENCE.get(agent, ()))
    if dataset_profile and dataset_profile in DATASET_PROFILES:
        preferred.extend(DATASET_PROFILES[dataset_profile].get("realized_fallback", ()))
    preferred.extend(
        [
            "forward_return_1m",
            "forward_return_5m",
    ...
    for col in preferred:
        ...
        if col in df.columns:
            return col
    available = [c for c in df.columns if str(c).startswith("forward_return_")]
    if available:
        return str(available[0])
    raise SystemExit(
        f"no forward_return_* column in dataset for agent={agent!r}; "
        "re-label with --horizon-preset scalp (HF) or swing_1h (klines)"
    )
...
def train_ppo(
    ...
    realized_col = _resolve_realized_col(df, agent, dataset_profile)
    mean, std, cols = _feature_stats(df)
    if not cols:
        raise SystemExit(
            "no PPO feature columns present in dataset — rebuild features / check PPO_FEATURE_COLS"
        )
    ...
    if holdout_path and Path(holdout_path).exists():
        ...
        hcol = _resolve_realized_col(hdf, agent, dataset_profile)
    ...
    logger.info(
        "training PPO[%s] profile=%s on %d rows (target=%s, feats=%d) for %d timesteps",
        agent, dataset_profile or "custom", len(df), realized_col, len(cols), timesteps,
    )
    model = PPO(
        "MlpPolicy",
        train_env,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--data", default=None,
                   help="labeled parquet; default comes from --dataset-profile")
    p.add_argument("--output", default=None, help="checkpoint path (.zip)")
    p.add_argument("--holdout", default=None,
                   help="holdout parquet; default comes from --dataset-profile")
    p.add_argument("--dataset-profile", default="klines",
                   choices=list(DATASET_PROFILES),
                   help="klines (swing) | hf (scalp HF tick)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    data_path = args.data or profile["data"]
    holdout_path = args.holdout or profile["holdout"]
    output = args.output or f"./models/agents/{args.agent}_{args.dataset_profile}_ppo.zip"
    train_ppo(
        agent=args.agent,
        data_path=data_path,
        output=output,
