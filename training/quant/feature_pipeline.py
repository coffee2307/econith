...
def _scan_asset_class(
    ...
    *,
    ...
    if only_subdirs:
        shards: list[Path] = []
        for name in only_subdirs:
            sub = base / name
            if sub.is_dir():
                shards.extend(sorted(sub.rglob("*.parquet")))
                shards.extend(sorted(sub.rglob("*.jsonl")))
            elif sub.is_file():
                shards.append(sub)
        logger.info(
            "raw tier '%s' filtered to subdirs %s (%d shards)",
            asset_class,
            only_subdirs,
            len(shards),
        )
...
def _build_macro_wide(macro: "pl.DataFrame") -> Optional["pl.DataFrame"]:
    """Pivot macro series into a ts-indexed wide frame of ``macro_*`` columns.

    Prefer one column per ``channel`` (semantic name from FRED/WB). Falls back to
    ``symbol`` when channel is missing. Payload-key discovery is a legacy path.
    """
    if macro.height == 0:
        return None

    valued = macro.with_columns(pl.col("value").cast(pl.Float64, strict=False)).filter(
        pl.col("value").is_not_null()
    )
    if valued.height == 0:
        return None

    # Stable column id: channel (fed_funds_...) else symbol (FEDFUNDS / USA_NY...).
    label = (
    ...
    valued = valued.with_columns(label)
    keys = sorted(
        k for k in valued.get_column("_macro_key").unique().to_list() if k and str(k).strip()
    )
    if not keys:
        return None

    # Cap width so a noisy lake cannot explode feature dim (keep densest series).
    if len(keys) > 80:
        counts = (
            valued.group_by("_macro_key")
            .len()
            .sort("len", descending=True)
            .head(80)
            .get_column("_macro_key")
            .to_list()
        )
        keys = [str(k) for k in counts]
        valued = valued.filter(pl.col("_macro_key").is_in(keys))

    frames: list["pl.DataFrame"] = []
    for key in keys:
        safe = "".join(ch if ch.isalnum() or ch == "_" else "_" for ch in str(key))
        column = f"macro_{safe}"
        sub = (
            valued.filter(pl.col("_macro_key") == key)
            .select([pl.col("ts_ms"), pl.col("value").alias(column)])
            .group_by("ts_ms")
    ...
    if not frames:
        # Legacy: numeric payload keys if present.
        payload_keys = _discover_payload_keys(macro)
        if not payload_keys:
            wide = (
        ...
        agg = [
            pl.col(f"macro_{key}").drop_nulls().last().alias(f"macro_{key}")
            for key in payload_keys
        ]
        return projected.group_by("ts_ms").agg(agg).sort("ts_ms")

    return _build_context(frames)
...
def run_pipeline(
    ...
    *,
    ...
    market_subdirs: Optional[list[str]] = None,
) -> dict:
    ...
    market = _scan_asset_class(raw_root, "market", only_subdirs=market_subdirs)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "comma-separated market child folders to scan "
            "(e.g. klines_hist). Empty = all. "
            "Use klines_hist to skip multi-GB tick desks on weak PCs."
        ),
    )
    return parser
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    market_subdirs = [s.strip() for s in args.market_subdirs.split(",") if s.strip()] or None
    summary = run_pipeline(
        raw_root=args.raw_root,
        out_dir=args.out_dir,
        symbols=symbols,
        loader_compat=not args.no_loader_compat,
        market_subdirs=market_subdirs,
    )
