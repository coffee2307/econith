      ...
      <div className="flex flex-none items-start justify-between gap-2 border-b border-line px-4 py-3">
        ...
        <span
          className={[
            ...
            live || policyLive ? "bg-emerald-500/10 text-emerald-600" : "bg-amber-500/10 text-amber-600",
          ].join(" ")}
        >
          <FontAwesomeIcon icon={faSatelliteDish} className="h-2.5 w-2.5" />
          {live || policyLive ? t("world.agentExchangeLive") : t("world.agentExchangeWaiting")}
        </span>
      </div>

      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        {lines.length === 0 ? (
          <p className="font-mono text-xs text-faint">{t("world.agentExchangeEmpty")}</p>
        ) : (
              ...
              >
                <div className="flex items-center justify-between gap-2 text-[10px] uppercase tracking-wider text-muted">
                  ...
                  <span className="font-mono text-faint">
                    ...
                    {line.simDay != null ? " · " : ""}
                    {new Date(line.ts).toLocaleTimeString(timeLocale, { hour12: false })}
                  </span>
                </div>
                <p className="mt-1.5 text-xs leading-relaxed text-ink">{line.text}</p>
              </article>
            ))}
          </div>
        )}
