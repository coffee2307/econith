...
#
#  v2 (2026-07): add HF microstructure columns from the VPS tick lake.
#  Older checkpoints trained on the v1 list still load via their `.norm.json`
#  (which stores its own column list), so this constant only affects NEW trains.
# ---------------------------------------------------------------------------
PPO_FEATURE_COLS: list[str] = [
    ...
    # HF microstructure (from training/quant/microstructure)
    "indicator_obi", "indicator_trade_imbalance", "indicator_vpin",
    "indicator_depth_imbalance", "indicator_book_slope",
    "spread", "rel_spread", "microprice",
]
