...
# The sovereign hub economies whose shocks propagate across the graph.
HUB_CODES: tuple[str, ...] = (
    "USA",
    "CHN",
    "VNM",
...
def macro_to_micro(world: WorldState) -> MicrostructuralVolatilityVector:
    ...
    vol_mult = _clamp(
        ...
        # Tight policy (high real rate) raises conditional vol — without this,
        # interest_rate shocks leave M_t / O_t unchanged because real_rate was unused.
        + 2.0 * max(0.0, geo.real_rate - 0.01),
        1.0, 8.0,
    )

    # Order-flow imbalance bias: risk-off (tension/inflation/tight policy) => net selling;
    # strong growth + confidence => net buying. Signed, saturating.
    obi_bias = _clamp(
        -(
            2.0 * geo.trade_tension
            + 1.4 * max(0.0, geo.inflation - 0.03)
            + 1.1 * geo.unrest
            + 1.5 * max(0.0, geo.real_rate - 0.01)
        )
