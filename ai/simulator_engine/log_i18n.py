...
def format_hypothesis_log(
    ...
    if outcome.status == "ok":
        ...
        # Prefer the concrete English prompt when present — less boilerplate.
        prompt = (hyp.prompt or "").strip()
        top = ""
        if outcome.deltas:
            items = sorted(
            ...
            for k, v in items:
                ...
                bits.append(f"{k} {sign}{float(v):.4f}")
            top = "; ".join(bits)
        if vi:
            base = f"Giả thuyết {hyp.id[:8]} · {summary}"
            if prompt:
                base += f" — «{prompt}»"
            if top:
                base += f" | Δ {top}"
            return base
        base = f"Hypothesis {hyp.id[:8]} · {summary}"
        if prompt:
            base += f" — «{prompt}»"
        if top:
            base += f" | Δ {top}"
...
def format_hypothesis_agent_line(
    hyp: "Hypothesis",
    outcome: "HypothesisOutcome",
    locale: str = "en",
) -> tuple[str, str, str]:
    """Return (actor, country, text) for the Agents tab — not Events."""
    vi = _is_vi(locale)
    country = (hyp.subjects[0] if hyp.subjects else "GLOBAL").upper()
    prompt = (hyp.prompt or hypothesis_summary(hyp, locale)).strip()
    if outcome.status != "ok":
        actor = "Government AI"
        if vi:
            text = f"Không thể kiểm thử «{prompt}»: {outcome.error or outcome.status}"
        else:
            text = f"Could not test «{prompt}»: {outcome.error or outcome.status}"
        return actor, country, text

    # Rotate speaker by category so the Agents tab is not all one voice.
    cat = (hyp.category or "generic").lower()
    if cat in ("tariff", "sanctions", "conflict"):
        actor = "Government AI"
    elif cat in ("stimulus", "sovereign_default", "monetary_tighten", "monetary_ease"):
        actor = "Central Bank" if "monetary" in cat else "Government AI"
    elif cat in ("pandemic",):
        actor = "Societal AI"
    else:
        actor = "Corporate AI"
    ...
    top = ""
    if outcome.deltas:
        k, v = max(outcome.deltas.items(), key=lambda kv: abs(float(kv[1])))
        sign = "+" if float(v) >= 0 else ""
        top = f"{k} {sign}{float(v):.4f}"

    sev = _sev_word(float(hyp.severity), vi=vi)
    if vi:
        text = (
            f"Đề xuất kiểm thử kịch bản mức {sev}: «{prompt}»"
            + (f" — phản ứng quan sát {top}." if top else ".")
        )
    else:
        text = (
            f"Proposing a {sev} scenario test: «{prompt}»"
            + (f" — observed response {top}." if top else ".")
        )
    return actor, country, text
