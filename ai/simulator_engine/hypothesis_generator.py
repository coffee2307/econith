...
class HypothesisGenerator:
    ...
    def _combinatorial(self, snapshot: dict[str, Any], codes: list[str]) -> Hypothesis:
        # Prefer major hubs when present so weak-PC combinatorial still feels global.
        hubs = [c for c in ("USA", "CHN", "DEU", "JPN", "IND", "VNM", "GBR", "FRA", "BRA") if c in codes]
        pool = hubs or codes
        code = self._rng.choice(pool)
        ...
        if cur is not None:
            # Small absolute step so the scientific loop always mutates a real dial.
            step = max(abs(cur) * 0.08, 0.002)
            target = cur + step if up else cur - step
            mutations = [{"country": code, "field": field, "value": round(target, 6)}]
        return Hypothesis(
            id=uuid.uuid4().hex[:12],
            prompt=prompt,
            rationale=rationale,
