...
class WorldKernel:
    ...
    async def _emit_all(self, facts: list[CausalFact], legacy_events: list[dict]) -> None:
        ...
        # Tab "Tác tử": ONLY sovereign/corporate/government/society (etc.) speakers.
        # Market / regime / tape→macro lines belong on the Events tab — mixing them
        # made both tabs look identical and drowned real agent debate.
        agent_facts = [f for f in ranked if f.actor in _AGENT_ACTORS]
        ...
        # Tab "Sự kiện": market regime, quant→macro contagion, policy headlines.
        ...
        if headline is None:
            market_or_system = [
                f for f in ranked if f.actor not in _AGENT_ACTORS or f.actor == "Market"
            ]
            pool = market_or_system or ranked
            for fact in pool:
                if self._narrative_gate_open(fact, severity, channel="headline"):
                    text = self._narrator.compose(fact, locale=locale)
                    # Prefer LLM voice for operator language (not string templates).
                    if (
                        self._governor_llm_pool is not None
                        and (locale or "").lower().startswith("vi")
                    ):
                        try:
                            ...
                            llm_text = await asyncio.to_thread(
                                compose_event_from_facts,
                                self._governor_llm_pool,
                                base_url=self._governor_llm_base_url or "",
                                model=self._governor_llm_model or "",
                                locale=locale,
                                facts={
                                    "actor": fact.actor,
                                    ...
                                    "effect": getattr(fact, "effect", None),
                                    "level": fact.level,
                                    "tags": list(getattr(fact, "tags", ()) or ()),
                                    "metrics": getattr(fact, "metrics", None),
                                },
                            )
                            if llm_text:
                                text = llm_text
                            ...
                            logger.debug(
                                "LLM headline compose failed; using narrator",
                                exc_info=True,
                            )
                    headline = {
                        "country": fact.country,
    ...
    _NARRATIVE_COOLDOWN_DAYS = 60
    _GLOBAL_TAG_COOLDOWN_DAYS = 35
    # Extra quiet period for the two spammiest market templates.
    _SPAMMY_TAG_COOLDOWN_DAYS = 90
    ...
    def _narrative_gate_open(
        ...
        tag_cooldown = self._GLOBAL_TAG_COOLDOWN_DAYS
        primary = str(tag).split(":", 1)[0]
        if primary in ("regime_shift", "quant_to_macro", "capital_flight") or str(tag).startswith(
            "regime:"
        ):
            tag_cooldown = self._SPAMMY_TAG_COOLDOWN_DAYS
        if (
            glast is not None
            and self._sim_day - glast[0] < tag_cooldown
            and rank <= glast[1]
        ):
            return False
    ...
    async def _emit_agent_narrative(self, fact: CausalFact, *, locale: str) -> None:
        # Hard guard: Market never enters the agent chat feed.
        if fact.actor not in _AGENT_ACTORS:
            return
        text_en = self._narrator.compose(fact, locale="en")
        text_vi = self._narrator.compose(fact, locale="vi")
        source = _ACTOR_SOURCE.get(fact.actor, "world")
        await self._bus.publish(
            "world.agent.narrative",
            sim_day=self._sim_day,
        ...
        if fact.level == "danger" and source != "regime":
            message = f"[{self._sim_date()}] {fact.country}: {text_en}"
            await self._bus.publish(
    ...
    def apply_mutations(self, mutations: list[dict]) -> list[str]:
        ...
        After each direct hit, a damped spillover is applied to other hubs using
        alliance strength + trade gravity so USA shocks are not USA/CHN-only.
        """
        applied: list[str] = []
        for mut in mutations:
            ...
            # Capture pre-value for spillover delta.
            if group is None:
                prev = float(getattr(c, real))
                setattr(c, real, _clamp_field(real, float(value)))
                new_val = float(getattr(c, real))
            else:
                raw_prev = c.get_field(group, real)
                prev = float(raw_prev) if raw_prev is not None else 0.0
                c.set_field(group, real, _clamp_field(real, float(value)))
                raw_new = c.get_field(group, real)
                new_val = float(raw_new) if raw_new is not None else float(value)
            applied.append(f"{c.name}.{field} -> {value}")
            delta = new_val - prev
            if abs(delta) < 1e-9:
                continue
            applied.extend(self._spillover_from_mutation(code, field, delta))
        return applied

    def _spillover_from_mutation(
        self, origin: str, field: str, delta: float
    ) -> list[str]:
        """Push a fraction of ``delta`` onto peer hubs via alliance + tariffs."""
        notes: list[str] = []
        origin_c = self._world.countries.get(origin)
        if origin_c is None or field not in LOGICAL_FIELDS:
            return notes
        # Multi-channel absorption so a USA rate hike moves peers globally.
        ...
        if field == "interest_rate":
            ...
            channels = [("inflation", 0.40), ("interest_rate", 0.25), ("gdp_growth", -0.20)]
        elif field == "gdp_growth":
        ...
        for code, peer in self._world.countries.items():
            ...
            for peer_field, scale in channels:
                if peer_field not in LOGICAL_FIELDS:
                    continue
                group, real = LOGICAL_FIELDS[peer_field]
                peer_delta = delta * weight * scale
                if abs(peer_delta) < 1e-8:
                    continue
                if group is None:
                    cur = float(getattr(peer, real))
                    setattr(peer, real, _clamp_field(real, cur + peer_delta))
                else:
                    raw = peer.get_field(group, real)
                    if raw is None:
                        continue
                    cur = float(raw)
