...
def _narratives(en: str, vi: str) -> dict[str, str]:
    """English source + Vietnamese display — UI selects; LLM path stays English."""
    return {"narrative": en, "narrative_vi": vi}
    ...
    def evaluate(self, state, world, ctx):
        ...
        if state.gdp_growth < 0.01 and state.corporate_tax > 0.10:
            proposals.append(PolicyProposal(
                ...
                **_narratives(
                    f"{state.name} cuts corporate tax to arrest the slowdown",
                    f"{state.name} cắt thuế doanh nghiệp để hãm đà suy giảm tăng trưởng",
                ),
            ))
        return proposals
    ...
    def evaluate(self, state, world, ctx):
        ...
        if tariffer and rate > 0.2:
            ...
            if alt:
                ...
                alt_name = world.node(alt).state.name
                pct = fraction * 100
                gain = 3.1 * rate
                proposals.append(PolicyProposal(
                    role=self.role, code=state.code, field="export_index",
                    ...
                    **_narratives(
                        f"{state.name} enterprises shift {pct:.0f}% of production "
                        f"to {alt_name} to bypass tariffs",
                        f"Doanh nghiệp {state.name} chuyển {pct:.0f}% sản xuất "
                        f"sang {alt_name} để né thuế quan",
                    ),
                ))
                ...
                proposals.append(PolicyProposal(
                    ...
                    **_narratives(
                        f"{alt_name} captures diverted supply-chain share "
                        f"(+{gain:.1f} export index)",
                        f"{alt_name} chiếm thêm thị phần chuỗi cung ứng "
                        f"(+{gain:.1f} chỉ số xuất khẩu)",
                    ),
                ))
        return proposals
...
class SovereignWorldGraph:
    ...
    def queue_tariff(self, source: str, target: str, rate: float) -> None:
        ...
        self._pending_mutations.append(PolicyProposal(
            ...
            **_narratives(
                f"{source} imposes a {rate*100:.0f}% import tariff on {target}",
                f"{source} áp thuế nhập khẩu {rate*100:.0f}% lên {target}",
            ),
        ))
    ...
    def queue_mutation(self, code: str, field: str, value: float) -> bool:
        ...
        code_u = code.upper()
        self._pending_mutations.append(PolicyProposal(
            role=AgentRole.GOVERNMENT, code=code_u, field=f"__set__:{field}",
            delta=value, reason="user_mutation",
            **_narratives(
                f"{code_u} sets {field} to {value:.4g}",
                f"{code_u} đặt {field} = {value:.4g}",
            ),
        ))
        return True
    ...
    async def _phase_apply_events(self, ctx: TickContext) -> None:
        for mut in self._pending_mutations:
            if mut.field == "__tariff__" and mut.tariff_edge:
                ...
                self._tick_facts.append((mut.narrative, mut.narrative_vi or mut.narrative))
        ...
        if self._pending_mutations:
            ...
            fork = self.chronology.fork(
                ...
                facts=[f[0] if isinstance(f, tuple) else f for f in self._tick_facts],
            )
    ...
    async def _phase_update(self, ctx: TickContext) -> None:
        if not get_system_controller().world_pipeline_active():
            return
        seen_en = {f[0] if isinstance(f, tuple) else f for f in self._tick_facts}
        for prop in self._tick_proposals:
            ...
            if prop.narrative and prop.narrative not in seen_en:
                self._tick_facts.append((prop.narrative, prop.narrative_vi or prop.narrative))
                seen_en.add(prop.narrative)
