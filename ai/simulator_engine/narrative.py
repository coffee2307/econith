...
class NarrativeEngine:
    ...
    def _compose_en(self, fact: CausalFact) -> str:
        ...
        if tag == "regime_shift" or any(t.startswith("regime:") for t in fact.tags):
            ...
            conf = m.get("confidence", 0.0)
            variants = (
                f"HMM flags a {regime} regime on {place} ({conf:.0%} confidence) — "
                f"quant desks are rebalancing risk budgets.",
                f"Market-mode classifier flipped to {regime} ({conf:.0%}) across {place}; "
                f"liquidity and systemic-risk pressures updated.",
                f"{place}: regime engine reads {regime} at {conf:.0%} confidence — "
                f"short-horizon capital is adjusting expectations.",
            )
        ...
        if tag == "quant_to_macro":
            ...
            bps = m.get("yield_shock_bps", 0.0)
            variants = (
                f"{place}: Quant→Macro channel fired — {self._money_en(usd)} fled, "
                f"yields +{bps:.0f} bps into the real balance sheet.",
                f"Tape stress leaked into {place}: net capital −{self._money_en(usd)}, "
                f"long-end yields up {bps:.0f} bps.",
                f"{place} absorbs a market shock: flight {self._money_en(usd)}, "
                f"yield curve +{bps:.0f} bp.",
            )
            return self._rng.choice(variants)
    ...
    def _compose_vi(self, fact: CausalFact) -> str:
        ...
        if tag == "regime_shift" or any(t.startswith("regime:") for t in fact.tags):
            ...
            variants = (
                f"HMM đánh dấu chế độ {regime_vi} tại {place} (xác suất {conf:.0%}) — "
                f"desk quant đang tái cân danh mục.",
                f"Phân loại chế độ chuyển sang {regime_vi} ({conf:.0%} tin cậy) trên {place}; "
                f"áp lực thanh khoản và rủi ro hệ thống được cập nhật.",
                f"{place}: bộ phân loại chế độ báo {regime_vi} với độ tin {conf:.0%} — "
                f"dòng vốn ngắn hạn đang điều chỉnh kỳ vọng.",
            )
            return self._rng.choice(variants)
        ...
        if tag == "quant_to_macro":
            ...
            bps = m.get("yield_shock_bps", 0.0)
            variants = (
                f"{place}: kênh Quant→Macro kích hoạt — rút {self._money_vi(usd)}, "
                f"lợi suất +{bps:.0f} điểm cơ bản lan sang cân đối thực.",
                f"Rủi ro thị trường thẩm thấu vào {place}: vốn ròng −{self._money_vi(usd)}, "
                f"lợi suất dài hạn tăng {bps:.0f} điểm cơ bản.",
                f"{place} hấp thụ cú shock từ tape: flight {self._money_vi(usd)}, "
                f"đường cong lợi suất dịch +{bps:.0f} bp.",
            )
            return self._rng.choice(variants)
