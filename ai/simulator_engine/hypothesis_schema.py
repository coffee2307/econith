...
class DebateTurn(BaseModel):
    """One utterance in the pre-mutation debate between world agents."""
...
class Hypothesis(BaseModel):
    ...
    # Explicit indicator targets from the scientific thinker (logical fields).
    mutations: list[dict[str, Any]] = Field(default_factory=list)
    created_at: datetime = Field(default_factory=_utc_now)
    generator: Literal["llm", "combinatorial"] = "combinatorial"
    # Scientific structure (optional — filled by the structured thinker).
    question: str = ""
    ...
    chapter: str = ""
    fingerprint: str = ""
    # Pre-mutation debate transcript + chair decision.
