...
AGENT_FEED_COOLDOWN_S = 35.0
# Market / regime headlines are noisier — keep Events readable.
REGIME_HEADLINE_COOLDOWN_S = 55.0

# Speakers allowed on the Agents (Tác tử) chat — never Market/regime templates.
_AGENT_FEED_ACTORS = frozenset({
    "Corporate AI",
    "Government AI",
    "Societal AI",
    "Sovereign",
...
class MetricsHub:
    ...
    async def _on_world_agent(self, event: Event) -> None:
        ...
        # Agents tab: sovereign speakers only — never Market/regime templates.
        if actor == "Market" or source == "regime":
            return
        provenance = str(event.payload.get("provenance") or "")
        is_hypothesis = provenance.startswith("hypothesis")
        if (
            actor not in _AGENT_FEED_ACTORS
            and source not in _AGENT_FEED_SOURCES
        ...
        # Scientific ask/act/judge must land as a triplet — don't throttle them away.
        cooldown = 0.4 if is_hypothesis else AGENT_FEED_COOLDOWN_S
        ...
        text_vi = str(event.payload.get("text_vi") or "").strip()
    ...
    async def _on_world_headline(self, event: Event) -> None:
        ...
        cooldown = (
            REGIME_HEADLINE_COOLDOWN_S
            if source in ("regime", "world")
            else AGENT_FEED_COOLDOWN_S
        )
        if now - self._headline_last_ts.get(headline_key, 0.0) < cooldown:
            return
        # Collapse near-duplicate capital-flight / regime lines (numbers drift).
        stem = message
        for needle in ("rút ", "flight ", "−", "+"):
            if needle in stem:
                stem = stem.split(needle)[0]
                break
        stem = stem[:72]
        last_stem_key = f"stem:{source}:{country}"
        if (
            self._headline_last_ts.get(last_stem_key, 0.0)
            and now - self._headline_last_ts.get(last_stem_key, 0.0) < cooldown
            and getattr(self, "_headline_last_stem", "") == stem
        ):
            return
        self._headline_last_ts[headline_key] = now
        self._headline_last_ts[last_stem_key] = now
        self._headline_last = message
        self._headline_last_stem = stem
        self._push_world_event(
            level=event.payload.get("level", "info"),
            source=source,
            message=message,
        ...
        self._world_agents.appendleft(
            {
                ...
                "text": text,
                "text_vi": str(lead.get("text_vi") or "").strip(),
