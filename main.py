...
from core.locale_prefs import dashboard_locale, set_dashboard_locale, bootstrap_locale_from_env
...
async def lifespan(app: FastAPI):
    env = get_environment()
    boot_locale = bootstrap_locale_from_env()
    logger.info("dashboard operator locale bootstrapped -> %s", boot_locale)
    ...
    hypothesis_runner = HypothesisRunner(
        ...
        generator=HypothesisGenerator(
            ...
            llm_pool=llm_pool,
            llm_base_url=(
                env.local_llm_base_url if env.local_llm_enabled else env.llm_base_url
            ),
            llm_model=(
                env.local_llm_model_name if env.local_llm_enabled else env.llm_model_name
            ),
            use_llm=True if llm_pool is not None else None,
        ),
