...
def extract_config_json_schema():
    try:
        ...
        from econith.config_schema import CONF_SCHEMA
    except ImportError:
        # If econith is not installed, add the parent directory to sys.path
        # to import directly from the source
        import sys

        script_dir = Path(__file__).parent
        econith_dir = script_dir.parent
        sys.path.insert(0, str(econith_dir))
