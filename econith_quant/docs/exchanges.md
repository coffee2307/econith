...
ECONITH Quant is based on [CCXT library](https://github.com/ccxt/ccxt) that supports over 100 cryptocurrency
...
If not enough `BNB` is available to cover transaction fees, then fees will not be covered by `BNB` and no fee reduction will occur. ECONITH Quant will never buy BNB to cover for fees. BNB needs to be bought and monitored manually to this end.
...
ECONITH Quant supports binance RSA API keys.

We recommend to use them as environment variable.

``` bash
export ECONITH_QUANT__EXCHANGE__SECRET="$(cat ./rsa_binance.private)"
```
...
ECONITH Quant will not attempt to change these settings.
...
ECONITH Quant will check these settings on startup, but won't attempt to change them.
...
The Kraken API does only provide 720 historic candles, which is sufficient for ECONITH Quant dry-run and live trade modes, but is a problem for backtesting.
...
These are usually updated once per quarter. ECONITH Quant expects these files to be placed in `user_data/data/kraken/trades_csv`.
...
!!! Note "Flex (Multi-collateral) Accounts"
    ...
    ECONITH Quant derives the `USD` balance from Kraken margin fields, so keep `stake_currency` set to `USD`.
...
!!! Warning "Futures"
    ...
    ECONITH Quant supports both modes (we recommend to use Buy/Sell mode) - but changing the mode mid-trading is not supported and will lead to exceptions and failures to place trades.
...
!!! Warning "Unified accounts"
    ECONITH Quant assumes accounts to be dedicated to the bot.
...
To use subaccounts with ECONITH Quant, you will need to use the following configuration pattern:
...
Hyperliquid allows you to create vaults. To use vaults with ECONITH Quant, you will need to use the following configuration pattern:
...
To trade on HIP-3 DEXes with ECONITH Quant, you need to add them to your configuration using the `hip3_dexes` parameter:
...
!!! Warning "Performance and Rate Limit Impact"
    ...
    * **Additional API Calls**: For each HIP-3 DEX configured, ECONITH Quant needs to make additional API calls.
...
By default, ECONITH Quant assumes that incomplete candle is fetched from the exchange and removes the last candle assuming it's the incomplete candle.

Whether your exchange returns incomplete candles or not can be checked using [the helper script](developer.md#incomplete-candles) from the Contributor documentation.

Due to the danger of repainting, ECONITH Quant does not allow you to use this incomplete candle.
...
### Advanced ECONITH Quant Exchange configuration
