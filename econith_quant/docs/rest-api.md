...
#### ECONITH Quant client- available commands
...
!!! Note
    The below should be done in an application (a ECONITH Quant REST API client, which fetches info via API), and is not intended to be used on a regular basis.

ECONITH Quant's REST API also offers JWT (JSON Web Tokens).
You can login using the following command, and subsequently use the resulting access_token.

``` bash
> curl -X POST --user ECONITH Quantr http://localhost:8080/api/v1/token/login
