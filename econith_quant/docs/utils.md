...
!!! Warning
    Only vital questions are asked. ECONITH Quant offers a lot more configuration possibilities, which are listed in the [Configuration documentation](configuration.md#configuration-parameters)
...
For pairs traded by ECONITH Quant the pair quote currency is defined by the value of the `stake_currency` configuration setting.
...
!!! Warning
    Please ensure to only use this on an empty target database. ECONITH Quant will perform a regular migration, but may fail if entries already existed.
