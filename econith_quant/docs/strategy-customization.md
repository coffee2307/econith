...
- the [ECONITH Quant strategy 101](strategy-101.md), which provides a quick start to strategy development
- the [ECONITH Quant bot basics](bot-basics.md), which provides overall info on how the bot operates
...
!!! Note
    There is a difference between the *name* of the strategy and the filename. In most commands, ECONITH Quant uses the *name* of the strategy, *not the filename*.
...
ECONITH Quant strategies can be processed by the ECONITH Quant bot in 5 main modes:
...
??? Hint "Lookahead and recursive analysis"
    ECONITH Quant includes two helpful commands to help assess common lookahead (using future data) and
...
ECONITH Quant uses [pandas](https://pandas.pydata.org/) to store/provide the candlestick (OHLCV) data.
...
ECONITH Quant uses this new column as an entry signal, where it is assumed that a trade will subsequently open on the next open candle.
...
ECONITH Quant does not store incomplete/unfinished candles in the dataframe.
...
ECONITH Quant does not. Only complete/finished candle data is available in the dataframe.
...
Some indicators have an unstable startup period in which there isn't enough candle data to calculate any values (NaN), or the calculation is incorrect. This can lead to inconsistencies, since ECONITH Quant does not know how long this unstable period is and uses whatever indicator values are in the dataframe.
...
ECONITH Quant locks pairs automatically for the current candle (until that candle is over) when a pair exits, preventing an immediate re-entry of that pair.
...
ECONITH Quant has an easy method to do this from within the strategy, by calling `self.lock_pair(pair, until, [reason])`.
