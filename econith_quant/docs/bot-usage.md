...
!!! Note
    If you've used `setup.sh`, don't forget to activate your virtual environment (`source .venv/bin/activate`) before running econith commands.
...
```bash
econith trade -c path/far/far/away/config.json
```
...
```bash
econith trade -c ./config.json
```

and specify both configuration files when running in the normal Live Trade Mode:

```bash
econith trade -c ./config.json -c path/to/secrets/keys.config.json
```
...
ECONITH Quant allows the creation of a user-data directory using `econith create-userdir --userdir someDirectory`.
...
```bash
econith trade --strategy AwesomeStrategy --strategy-path /some/directory
```
...
```bash
econith trade -c config.json --db-url sqlite:///tradesv3.dry_run.sqlite
```
