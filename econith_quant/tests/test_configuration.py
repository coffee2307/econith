...
def test_validate_edge_removal(default_conf):
    ...
    with pytest.raises(
        ConfigurationError,
        match=r"Edge is no longer supported and has been removed from ECONITH Quant with 2025\.6\.",
    ):
        validate_config_consistency(default_conf)
...
def test_flat_vars_to_nested_dict(caplog):
    test_args = {
        "ECONITH_QUANT__EXCHANGE__SOME_SETTING": "true",
        "ECONITH_QUANT__EXCHANGE__SOME_FALSE_SETTING": "false",
        "ECONITH_QUANT__EXCHANGE__CONFIG__whatEver": "sometime",  # Lowercased
        # Preserve case for ccxt_config
        "ECONITH_QUANT__EXCHANGE__CCXT_CONFIG__httpsProxy": "something",
        "ECONITH_QUANT__EXIT_PRICING__PRICE_SIDE": "bid",
        "ECONITH_QUANT__EXIT_PRICING__cccc": "500",
        "ECONITH_QUANT__STAKE_AMOUNT": "200.05",
        "ECONITH_QUANT__TELEGRAM__CHAT_ID": "2151",
        "NOT_RELEVANT": "200.0",  # Will be ignored
        "ECONITH_QUANT__ARRAY": '[{"name":"default","host":"xxx"}]',
        "ECONITH_QUANT__EXCHANGE__PAIR_WHITELIST": '["BTC/USDT", "ETH/USDT"]',
        # Fails due to trailing comma
        "ECONITH_QUANT__ARRAY_TRAIL_COMMA": '[{"name":"default","host":"xxx",}]',
        # Object fails
        "ECONITH_QUANT__OBJECT": '{"name":"default","host":"xxx"}',
    }
    expected = {
        "stake_amount": 200.05,
        "exit_pricing": {
    ...
    assert log_has("Loading variable 'ECONITH_QUANT__EXCHANGE__SOME_SETTING'", caplog)
