...
from econith.enums import ExitCheckTuple, ExitType, RPCMessageType
from econith.exceptions import ExchangeError, InsufficientFundsError, InvalidOrderException
from econith.econithbot import EconithBot
from econith.persistence import Order, Trade
from econith.persistence.models import PairLock
...
from tests.econithbot.test_econithbot import patch_RPCManager
...
def test_handle_stoploss_on_exchange(
    ...
    assert econith.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    ...
    mocker.patch.object(econith.exchange, "fetch_stoploss_order", hanging_stoploss_order)
    ...
    mocker.patch.object(econith.exchange, "fetch_stoploss_order", canceled_stoploss_order)
    stoploss.reset_mock()
    ...
    mocker.patch.object(econith.exchange, "fetch_stoploss_order", stoploss_order_hit)
    econith.strategy.order_filled = MagicMock(return_value=None)
    assert econith.handle_stoploss_on_exchange(trade) is True
    ...
    mocker.patch.object(econith.exchange, "create_stoploss", side_effect=ExchangeError())
    trade.is_open = True
    econith.handle_stoploss_on_exchange(trade)
    ...
    stoploss.reset_mock()
    mocker.patch.multiple(econith.exchange, fetch_order=MagicMock(), create_stoploss=stoploss)
    assert econith.handle_stoploss_on_exchange(trade) is False
    assert trade.has_open_sl_orders is False
...
def test_handle_stoploss_on_exchange_emergency(
    ...
    econith.config["trailing_stop"] = True
...
def test_handle_stoploss_on_exchange_partial(
    ...
    mocker.patch.multiple(econith.exchange, fetch_stoploss_order=stoploss_order_hit)
    assert econith.handle_stoploss_on_exchange(trade) is False
...
def test_handle_stoploss_on_exchange_partial_cancel_here(
    ...
    # ECONITH Quant cancels the stop - but cancel returns a partial filled order.
    stoploss_order_hit = MagicMock(
        return_value={
            "id": "101",
            "status": "open",
...
def test_create_stoploss_order_invalid_order(
    ...
    caplog.clear()
    rpc_mock.reset_mock()
    econith.create_stoploss_order(trade, 200)
    assert trade.has_open_sl_orders is False
...
def test_handle_stoploss_on_exchange_trailing(
    ...
    # setting stoploss
    econith.strategy.stoploss = 0.05 if is_short else -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    econith.strategy.order_types["stoploss_on_exchange_interval"] = 60
    ...
    stoploss_order_mock.assert_called_once_with(
        amount=30,
        pair="ETH/USDT",
        order_types=econith.strategy.order_types,
...
def test_handle_stoploss_on_exchange_trailing_error(
    ...
    mocker.patch.object(
        econith.exchange, "cancel_stoploss_order", side_effect=InvalidOrderException()
    )
    mocker.patch.object(
        econith.exchange, "fetch_stoploss_order", return_value=stoploss_order_hanging
    )
    time_machine.shift(timedelta(minutes=50))
    econith.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
    ...
    caplog.clear()
    cancel_mock = mocker.patch.object(econith.exchange, "cancel_stoploss_order")
    mocker.patch.object(econith.exchange, "create_stoploss", side_effect=ExchangeError())
    time_machine.shift(timedelta(minutes=50))
    econith.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_stoploss_on_exchange_price_rounding(
    ...
    econith.handle_trailing_stoploss_on_exchange(open_trade_usdt, {})
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
    econith.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
    ...
    econith.strategy.order_types["stoploss_on_exchange_interval"] = 0
    cancel_order_mock.assert_not_called()
    stoploss_order_mock.assert_not_called()
...
def test_execute_trade_exit_sloe_cancel_exception(
    ...
    mocker.patch("econith.wallets.Wallets.get_free", MagicMock(return_value=300))
    create_order_mock = MagicMock(
        side_effect=[
            {"id": "12345554"},
            {"id": "12345555"},
...
def test_may_execute_trade_exit_after_stoploss_on_exchange_hit(
    ...
    econith = EconithBot(default_conf_usdt)
    econith.strategy.order_types["stoploss_on_exchange"] = True
    patch_get_signal(econith, enter_long=not is_short, enter_short=is_short)
    ...
    mocker.patch.object(econith.exchange, "fetch_stoploss_order", stoploss_executed)
