...
def test_worker_state(mocker, default_conf, markets) -> None:
    ...
    assert worker.econith.state is State.STOPPED


def test_worker_running(mocker, default_conf, caplog) -> None:
    mock_throttle = MagicMock()
    mocker.patch("econith.worker.Worker._throttle", mock_throttle)
    mocker.patch("econith.persistence.Trade.stoploss_reinitialization", MagicMock())
    ...
    assert worker.econith.strategy
    assert worker.econith.strategy.dp
    assert isinstance(worker.econith.strategy.dp, DataProvider)
...
def test_worker_paused(mocker, default_conf, caplog) -> None:
    ...
    worker.econith.state = State.PAUSED
...
def test_worker_stopped(mocker, default_conf, caplog) -> None:
    ...
    worker = get_patched_worker(mocker, default_conf)
    worker.econith.state = State.STOPPED
    state = worker._worker(old_state=State.RUNNING)
...
def test_worker_lifecycle(
    ...
    mocker.patch("econith.worker.Worker._throttle", mock_throttle)
    mocker.patch("econith.persistence.Trade.stoploss_reinitialization")
    startup = mocker.patch("econith.econithbot.EconithBot.startup")

    worker = get_patched_worker(mocker, default_conf)
    worker.econith.state = target_state
...
def test_throttle_sleep_time(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    worker = get_patched_worker(mocker, default_conf)
    sleep_mock = mocker.patch("econith.worker.Worker._sleep")
...
def test_worker_heartbeat_running(default_conf, mocker, caplog):
    ...
    worker.econith.state = State.RUNNING
