...
from econith.constants import CANCEL_REASON, UNLIMITED_STAKE_AMOUNT
from econith.enums import (
    CandleType,
    ExitCheckTuple,
    ExitType,
    RPCMessageType,
...
from econith.plugins.protections.iprotection import ProtectionReturn
from econith.util.datetime_helpers import dt_now, dt_utc
...
def patch_RPCManager(mocker) -> MagicMock:
    ...
    rpc_mock = mocker.patch("econith.econithbot.RPCManager.send_msg", MagicMock())
    return rpc_mock
...
def test_econithbot_state(mocker, default_conf_usdt, markets) -> None:
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    econith = get_patched_econithbot(mocker, default_conf_usdt)
    assert econith.state is State.RUNNING

    default_conf_usdt.pop("initial_state")
    econith = EconithBot(default_conf_usdt)
    assert econith.state is State.STOPPED


def test_process_stopped(mocker, default_conf_usdt) -> None:
    econith = get_patched_econithbot(mocker, default_conf_usdt)
    coo_mock = mocker.patch("econith.econithbot.EconithBot.cancel_all_open_orders")
    econith.process_stopped()
...
def test_process_calls_sendmsg(mocker, default_conf_usdt) -> None:
    econith = get_patched_econithbot(mocker, default_conf_usdt)
    econith.process()
    assert econith.rpc.process_msg_queue.call_count == 1


def test_bot_cleanup(mocker, default_conf_usdt, caplog) -> None:
    mock_cleanup = mocker.patch("econith.econithbot.Trade.commit")
    coo_mock = mocker.patch("econith.econithbot.EconithBot.cancel_all_open_orders")
    econith = get_patched_econithbot(mocker, default_conf_usdt)
    econith.cleanup()
    ...
    econith.config["cancel_open_orders_on_exit"] = True
    econith.cleanup()
    assert coo_mock.call_count == 1


def test_bot_cleanup_db_errors(mocker, default_conf_usdt, caplog) -> None:
    mocker.patch("econith.econithbot.Trade.commit", side_effect=OperationalException())
    mocker.patch(
        "econith.econithbot.EconithBot.check_for_open_trades",
        side_effect=OperationalException(),
    )
    econith = get_patched_econithbot(mocker, default_conf_usdt)
    econith.emc = MagicMock()
    econith.emc.shutdown = MagicMock()
    econith.cleanup()
    assert econith.emc.shutdown.call_count == 1
...
def test_order_dict(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    econith = EconithBot(conf)
    if runmode == RunMode.LIVE:
        assert not log_has_re(r".*stoploss_on_exchange .* dry-run", caplog)
    assert econith.strategy.order_types["stoploss_on_exchange"]
    ...
    econith = EconithBot(conf)
    assert not econith.strategy.order_types["stoploss_on_exchange"]
    assert not log_has_re(r".*stoploss_on_exchange .* dry-run", caplog)
...
def test_get_trade_stake_amount(default_conf_usdt, mocker) -> None:
    ...
    result = econith.wallets.get_trade_stake_amount("ETH/USDT", 1)
...
def test_load_strategy_no_keys(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    conf["runmode"] = runmode
    erm = mocker.patch("econith.econithbot.ExchangeResolver.load_exchange")

    econith = EconithBot(conf)
    strategy_config = econith.strategy.config
...
def test_check_available_stake_amount(
    ...
    for i in range(0, max_open):
        if expected[i] is not None:
            ...
            econith.execute_entry("ETH/USDT", result)
        else:
            with pytest.raises(DependencyException):
                econith.wallets.get_trade_stake_amount("ETH/USDT", 1)
...
def test_create_trade_minimal_amount(
    ...
    econith = EconithBot(default_conf_usdt)
    econith.config["stake_amount"] = stake_amount
    patch_get_signal(econith, enter_short=is_short, enter_long=not is_short)

    if create:
        assert econith.create_trade("ETH/USDT")
        if amount_enough:
        ...
        assert not econith.create_trade("ETH/USDT")
        if not max_open_trades:
            assert (
                econith.wallets.get_trade_stake_amount(
...
def test_enter_positions_no_pairs_left(
    ...
    mocker.patch("econith.configuration.config_validation._validate_whitelist")
...
def test_handle_protections(mocker, default_conf_usdt, fee, is_short):
    ...
    econith = get_patched_econithbot(mocker, default_conf_usdt)
    econith.protections._protection_handlers[1].global_stop = MagicMock(
        return_value=ProtectionReturn(True, dt_now() + timedelta(hours=1), "asdf")
    )
    create_mock_trades(fee, is_short)
    econith.handle_protections("ETC/BTC", "*")
    send_msg_mock = econith.rpc.send_msg
...
def test_create_trade_no_signal(default_conf_usdt, fee, mocker) -> None:
    ...
    econith = EconithBot(default_conf_usdt)
    patch_get_signal(econith, enter_long=False, exit_long=False)
...
def test_create_trades_preopen(
    ...
    econith.execute_entry("ETH/USDT", default_conf_usdt["stake_amount"])
    econith.execute_entry("NEO/BTC", default_conf_usdt["stake_amount"])
    ...
    assert econith.create_trade("ETH/USDT")
    assert econith.create_trade("NEO/BTC")
...
def test_process_trade_creation(
    ...
    mocker.patch("econith.econithbot.EconithBot._check_and_execute_exit")
...
def test_process_exchange_failures(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    patch_get_signal(worker.econith)
...
def test_process_operational_exception(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    assert worker.econith.state == State.RUNNING

    worker._process_running()
    assert worker.econith.state == State.STOPPED
...
def test_process_trade_no_whitelist_pair(
    ...
    assert pair not in econith.active_pair_whitelist
    econith.process()
    assert pair in econith.active_pair_whitelist
    # Make sure each pair is only in the list once
    assert len(econith.active_pair_whitelist) == len(set(econith.active_pair_whitelist))
...
def test_process_informative_pairs_added(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    mocker.patch.multiple(
        "econith.strategy.interface.IStrategy",
    ...
    econith = EconithBot(default_conf_usdt)
    econith.strategy.informative_pairs = inf_pairs
    # patch_get_signal(econith)
...
def test_execute_entry(
    ...
    econith = EconithBot(default_conf_usdt)
    econith.strategy.confirm_trade_entry = MagicMock(return_value=False)
    ...
    mocker.patch.multiple(
        "econith.exchange.okx.Okx",
        get_max_pair_stake_amount=MagicMock(return_value=500000),
    )
    pair = "ETH/USDT"

    assert not econith.execute_entry(pair, stake_amount, is_short=is_short)
    assert enter_rate_mock.call_count == 1
    assert enter_mm.call_count == 0
    assert econith.strategy.confirm_trade_entry.call_count == 1
    enter_rate_mock.reset_mock()

    open_order["id"] = "22"
    econith.strategy.confirm_trade_entry = MagicMock(return_value=True)
    assert econith.execute_entry(pair, stake_amount)
    ...
    open_order["id"] = "33"
    fix_price = 0.06
    assert econith.execute_entry(pair, stake_amount, fix_price, is_short=is_short)
    ...
    assert econith.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    econith.strategy.custom_stake_amount = lambda **kwargs: 150.0
    assert econith.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    # Exception case
    order["id"] = "557"
    econith.strategy.custom_stake_amount = lambda **kwargs: 20 / 0
    assert econith.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order["id"] = "66"
    mocker.patch(f"{EXMS}.create_order", MagicMock(return_value=order))
    assert not econith.execute_entry(pair, stake_amount)
    assert econith.strategy.leverage.call_count == 0 if trading_mode == "spot" else 2
    ...
    with pytest.raises(PricingError, match=r"Could not determine entry price\."):
        econith.execute_entry(pair, stake_amount, is_short=is_short)

    # In case of custom entry price
    mocker.patch(f"{EXMS}.get_rate", return_value=0.50)
    order["status"] = "open"
    order["id"] = "5566"
    econith.strategy.custom_entry_price = lambda **kwargs: 0.508
    assert econith.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order["status"] = "open"
    order["id"] = "5567"
    econith.strategy.custom_entry_price = lambda **kwargs: None
    ...
    order["status"] = "open"
    order["id"] = "5568"
    econith.strategy.custom_entry_price = lambda **kwargs: "string price"
    assert econith.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    econith.exchange.get_max_pair_stake_amount = MagicMock(return_value=500)

    assert econith.execute_entry(pair, 2000, is_short=is_short)
    ...
    econith.strategy.leverage.reset_mock()
    assert econith.execute_entry(pair, 200, leverage_=3)
    assert econith.strategy.leverage.call_count == 0
...
def test_execute_entry_confirm_error(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    econith.strategy.confirm_trade_entry = MagicMock(side_effect=ValueError)
    assert econith.execute_entry(pair, stake_amount)

    limit_order[entry_side(is_short)]["id"] = "222"
    econith.strategy.confirm_trade_entry = MagicMock(side_effect=Exception)
    assert econith.execute_entry(pair, stake_amount)
...
def test_execute_entry_fully_canceled_on_create(
    ...
    mock_hce = mocker.spy(econith, "handle_cancel_enter")
...
def test_execute_entry_min_leverage(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    stake_amount = 2
    pair = "SOL/BUSD:BUSD"
    econith.strategy.leverage = MagicMock(return_value=5.0)
...
def test_enter_positions(
    ...
    mock_ct = mocker.patch(
        "econith.econithbot.EconithBot.create_trade",
...
def test_exit_positions(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch("econith.econithbot.EconithBot.handle_trade", MagicMock(return_value=True))
    ...
    Trade.commit()
    trades = [trade]
    econith.wallets.update()
    n = econith.exit_positions(trades)
    assert n == 1
    # Test amount not modified by fee-logic
    assert not log_has_re(r"Applying fee to amount for Trade .*", caplog)

    gra = mocker.patch("econith.econithbot.EconithBot.get_real_amount", return_value=0.0)
...
def test_exit_positions_exception(mocker, default_conf_usdt, limit_order, caplog, is_short) -> None:
    ...
    mocker.patch(
        "econith.econithbot.EconithBot.handle_trade", side_effect=DependencyException()
    )
    caplog.clear()
    n = econith.exit_positions(trades)
    assert n == 0
...
def test_update_trade_state(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch("econith.econithbot.EconithBot.handle_trade", MagicMock(return_value=True))
    mocker.patch("econith.econithbot.EconithBot._notify_enter")
    mocker.patch(f"{EXMS}.fetch_order", return_value=order)
    mocker.patch(f"{EXMS}.get_trades_for_order", return_value=[])
    mocker.patch("econith.econithbot.EconithBot.get_real_amount", return_value=0.0)
    order_id = order["id"]
    ...
    econith.strategy.order_filled = MagicMock(return_value=None)
    assert not econith.update_trade_state(trade, None)
    assert log_has_re(r"Orderid for trade .* is empty.", caplog)
    caplog.clear()
    # Add datetime explicitly since sqlalchemy defaults apply only once written to database
    econith.update_trade_state(trade, order_id)
    # Test amount not modified by fee-logic
    assert not log_has_re(r"Applying fee to .*", caplog)
    caplog.clear()
    assert not trade.has_open_orders
    assert trade.amount == order["amount"]
    assert econith.strategy.order_filled.call_count == 1

    mocker.patch("econith.econithbot.EconithBot.get_real_amount", return_value=0.01)
    assert trade.amount == 30.0
    # test amount modified by fee-logic
    econith.update_trade_state(trade, order_id)
    ...
    econith.strategy.order_filled = MagicMock(return_value=None)
    mocker.patch("econith.econithbot.EconithBot.get_real_amount", side_effect=ValueError)
    mocker.patch(f"{EXMS}.fetch_order", return_value=limit_buy_order_usdt_new)
    res = econith.update_trade_state(trade, order_id)
    # Cancelled empty
    assert res is True
    assert econith.strategy.order_filled.call_count == 0
...
def test_update_trade_state_withorderdict(
    ...
    econith.update_trade_state(trade, order_id, order)
...
def test_update_trade_state_exception(
    ...
    mocker.patch(
        "econith.econithbot.EconithBot.get_real_amount", side_effect=DependencyException()
    )
    econith.update_trade_state(trade, open_order_id)
...
def test_update_trade_state_orderexception(mocker, default_conf_usdt, caplog) -> None:
    ...
    # Test raise of OperationalException exception
    grm_mock = mocker.patch("econith.econithbot.EconithBot.get_real_amount", MagicMock())
    econith.update_trade_state(trade, open_order_id)
...
def test_update_trade_state_sell(
    ...
    mocker.patch("econith.wallets.Wallets.update", wallet_mock)
    ...
    econith.update_trade_state(trade, trade.open_orders_ids[-1], l_order)
...
def test_handle_trade(
    ...
    assert econith.handle_trade(trade) is True
...
def test_handle_overlapping_signals(
    ...
    econith = EconithBot(default_conf_usdt)
    if is_short:
        patch_get_signal(econith, enter_long=False, enter_short=True, exit_short=True)
    else:
        patch_get_signal(econith, enter_long=True, exit_long=True)
    econith.strategy.min_roi_reached = MagicMock(return_value=False)
    ...
    patch_get_signal(econith, enter_long=False)
    assert econith.handle_trade(trades[0]) is False
    trades = Trade.session.scalars(select(Trade)).all()
    for trade in trades:
        trade.is_short = is_short
    nb_trades = len(trades)
    ...
    for trade in trades:
        trade.is_short = is_short
    assert econith.handle_trade(trades[0]) is True
...
def test_handle_trade_roi(
    ...
    econith = get_patched_econithbot(mocker, default_conf_usdt)
    patch_get_signal(econith, enter_short=is_short, enter_long=not is_short)
    econith.strategy.min_roi_reached = MagicMock(return_value=True)
    ...
    caplog.clear()
    patch_get_signal(econith)
    assert econith.handle_trade(trade)
...
def test_handle_trade_use_exit_signal(
    ...
    patch_get_signal(econith, enter_long=False, exit_long=False)
    assert not econith.handle_trade(trade)
...
def test_close_trade(
    ...
    with pytest.raises(DependencyException, match=r".*closed trade.*"):
        econith.handle_trade(trade)


def test_bot_loop_start_called_once(mocker, default_conf_usdt, caplog):
    ftbot = get_patched_econithbot(mocker, default_conf_usdt)
    mocker.patch("econith.econithbot.EconithBot.create_trade")
...
def test_manage_open_orders_entry_usercustom(
    ...
    econith.strategy.check_entry_timeout = MagicMock(return_value=False)
    econith.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    ...
    nb_trades = len(trades)
    assert nb_trades == 1
    assert econith.strategy.check_entry_timeout.call_count == 1
    econith.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    assert nb_trades == 1
    assert econith.strategy.check_entry_timeout.call_count == 1
    econith.strategy.check_entry_timeout = MagicMock(return_value=True)
...
def test_manage_open_orders_entry(
    ...
    assert econith.strategy.check_entry_timeout.call_count == 0
...
def test_adjust_entry_cancel(
    ...
    econith.strategy.adjust_entry_price = MagicMock(return_value=None)
    econith.manage_open_orders()
...
def test_adjust_entry_replace_fail(
    ...
    mocker.patch("econith.econithbot.sleep")
    ...
    econith.strategy.adjust_entry_price = MagicMock(return_value=12234)
    econith.manage_open_orders()
    trades = Trade.session.scalars(select(Trade).where(Order.ft_trade_id == Trade.id)).all()
...
def test_adjust_entry_replace_fail_create_order(
    ...
    mocker.patch("econith.econithbot.sleep")
    mocker.patch(
        "econith.econithbot.EconithBot.execute_entry", side_effect=DependencyException()
    )
...
def test_adjust_entry_maintain_replace(
    ...
    econith.strategy.adjust_entry_price = MagicMock(return_value=old_order["price"])
    econith.manage_open_orders()
    trades = Trade.session.scalars(
    ...
    econith.get_valid_enter_price_and_stake = MagicMock(return_value={100, 10, 1})
    econith.strategy.adjust_entry_price = MagicMock(return_value=1234)
...
def test_manage_open_orders_exit_usercustom(
    ...
    et_mock = mocker.patch("econith.econithbot.EconithBot.execute_trade_exit")
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
    ...
    econith.strategy.check_exit_timeout = MagicMock(return_value=False)
    econith.strategy.check_entry_timeout = MagicMock(return_value=False)
    # Return false - No impact
    econith.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    assert rpc_mock.call_count == 1
    assert econith.strategy.check_exit_timeout.call_count == 1
    assert econith.strategy.check_entry_timeout.call_count == 0

    econith.strategy.check_exit_timeout = MagicMock(side_effect=KeyError)
    econith.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    # Return True - sells!
    econith.strategy.check_exit_timeout = MagicMock(return_value=True)
    econith.strategy.check_entry_timeout = MagicMock(return_value=True)
    econith.manage_open_orders()
    assert cancel_order_mock.call_count == 1
    assert rpc_mock.call_count == 2
    ...
    mocker.patch("econith.persistence.Trade.get_canceled_exit_order_count", return_value=1)
    mocker.patch(
        "econith.econithbot.EconithBot.execute_trade_exit", side_effect=DependencyException
    )
    econith.manage_open_orders()
    ...
    with patch("econith.econithbot.EconithBot.handle_cancel_exit", return_value=False):
        econith.manage_open_orders()
...
def test_manage_open_orders_exit(
    ...
    assert econith.strategy.check_exit_timeout.call_count == 0
    assert econith.strategy.check_entry_timeout.call_count == 0
...
def test_manage_open_orders_partial_fee(
    ...
    mocker.patch("econith.wallets.Wallets.get_free", MagicMock(return_value=0))
    patch_exchange(mocker)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
...
def test_manage_open_orders_partial_except(
    ...
    mocker.patch(
        "econith.econithbot.EconithBot.get_real_amount",
...
def test_handle_cancel_enter(mocker, caplog, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    econith = EconithBot(default_conf_usdt)
    econith._notify_enter_cancel = MagicMock()
    ...
    assert econith.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 1

    cancel_order_mock.reset_mock()
    caplog.clear()
    l_order["filled"] = 0.01
    assert not econith.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 0
    ...
    assert not econith.handle_cancel_enter(
...
def test_handle_cancel_enter_exchanges(
    ...
    notify_mock = mocker.patch("econith.econithbot.EconithBot._notify_enter_cancel")
    econith = EconithBot(default_conf_usdt)
    ...
    Trade.session.add(trade)
    Trade.commit()
    assert econith.handle_cancel_enter(
...
def test_handle_cancel_exit_limit(
    ...
    mocker.patch("econith.econithbot.EconithBot.handle_order_fee")
    ...
    send_msg_mock.reset_mock()
    assert econith.handle_cancel_exit(trade, order, order_obj, reason)
    assert cancel_order_mock.call_count == 1
    assert send_msg_mock.call_count == 1
    ...
    assert not econith.handle_cancel_exit(trade, order, order_obj, reason)
...
def test_handle_cancel_exit_cancel_exception(mocker, default_conf_usdt) -> None:
    ...
    # assert not econith.handle_cancel_exit(trade, order, reason)
...
def test_execute_trade_exit_up(
    ...
    econith = EconithBot(default_conf_usdt)
    patch_get_signal(econith, enter_short=is_short, enter_long=not is_short)
    econith.strategy.confirm_trade_exit = MagicMock(return_value=False)
    ...
    assert id(econith.strategy.confirm_trade_exit.call_args_list[0][1]["trade"]) != id(trade)
    assert econith.strategy.confirm_trade_exit.call_args_list[0][1]["trade"].id == trade.id
...
def test_execute_trade_exit_custom_exit_price(
    ...
    econith.strategy.custom_exit_price = lambda **kwargs: 2.25
    econith.execute_trade_exit(
        trade=trade,
...
def test_execute_trade_exit_market_order(
    ...
    econith.config["order_types"]["exit"] = "market"
...
def test_execute_trade_exit_insufficient_funds_error(
    ...
    econith = get_patched_econithbot(mocker, default_conf_usdt)
    mock_insuf = mocker.patch("econith.econithbot.EconithBot.handle_insufficient_funds")
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        get_fee=fee,
    ...
    assert not econith.execute_trade_exit(
        trade=trade,
        limit=ticker_usdt_sell_up()["ask" if is_short else "bid"],
...
def test_exit_profit_only(
    ...
    econith = EconithBot(default_conf_usdt)
    patch_get_signal(econith, enter_short=is_short, enter_long=not is_short)
    econith.strategy.custom_exit = MagicMock(return_value=None)
    if exit_type == ExitType.EXIT_SIGNAL.value:
        econith.strategy.min_roi_reached = MagicMock(return_value=False)
    else:
        econith.strategy.ft_stoploss_reached = MagicMock(
    ...
    econith.wallets.update()
    if profit_only:
        assert econith.handle_trade(trade) is False
        # Custom-exit is called
        assert econith.strategy.custom_exit.call_count == 1

    patch_get_signal(econith, enter_long=False, exit_short=is_short, exit_long=not is_short)
    assert econith.handle_trade(trade) is handle_first

    if handle_second:
        econith.strategy.exit_profit_offset = 0.0
        assert econith.handle_trade(trade) is True
...
def test_sell_not_enough_balance(
    ...
    trade.update_trade(oobj)
    patch_get_signal(econith, enter_long=False, exit_long=True)
    mocker.patch("econith.wallets.Wallets.get_free", MagicMock(return_value=trade.amount * 0.985))
...
def test__safe_exit_amount(default_conf_usdt, fee, caplog, mocker, amount_wallet, has_err):
    ...
    mocker.patch("econith.wallets.Wallets.get_free", MagicMock(return_value=amount_wallet))
    wallet_update = mocker.patch("econith.wallets.Wallets.update")
    trade = Trade(
        pair="LTC/ETH",
        amount=amount,
        exchange="binance",
    ...
    if has_err:
        with pytest.raises(DependencyException, match=r"Not enough amount to exit trade."):
            assert econith._safe_exit_amount(trade, trade.pair, trade.amount)
    else:
        wallet_update.reset_mock()
        assert trade.amount != amount_wallet
        assert econith._safe_exit_amount(trade, trade.pair, trade.amount) == amount_wallet
        assert log_has_re(r".*Falling back to wallet-amount.", caplog)
        assert trade.amount == amount_wallet
        assert wallet_update.call_count == 1
        caplog.clear()
        wallet_update.reset_mock()
        assert econith._safe_exit_amount(trade, trade.pair, amount_wallet) == amount_wallet
...
def test_locked_pairs(
    ...
    assert not econith.strategy.is_pair_locked(trade.pair, side="*")
    # Both sides are locked
    assert econith.strategy.is_pair_locked(trade.pair, side="long") != is_short
    assert econith.strategy.is_pair_locked(trade.pair, side="short") == is_short
...
def test_ignore_roi_if_entry_signal(
    ...
    if is_short:
        patch_get_signal(econith, enter_long=False, exit_short=False, exit_tag="something")
    else:
        patch_get_signal(econith, enter_long=False, exit_long=False, exit_tag="something")
    assert econith.handle_trade(trade) is True
...
def test_disable_ignore_roi_if_entry_signal(
    ...
    patch_get_signal(econith, enter_long=not is_short, enter_short=is_short, exit_short=is_short)
    assert econith.handle_trade(trade) is True
...
def test_get_real_amount_quote(
    ...
    assert econith.get_real_amount(trade, buy_order_fee, order_obj) == (amount * 0.001)
    assert log_has(
...
def test_get_real_amount_quote_dust(
    ...
    mocker.patch(f"{EXMS}.get_trades_for_order", return_value=trades_for_order)
    walletmock = mocker.patch("econith.wallets.Wallets.update")
    mocker.patch("econith.wallets.Wallets.get_free", return_value=8.1122)
    amount = sum(x["amount"] for x in trades_for_order)
    trade = Trade(
        pair="LTC/ETH",
        amount=amount,
    ...
    assert econith.get_real_amount(trade, buy_order_fee, order_obj) is None
...
def test_get_real_amount(
    ...
    caplog.clear()
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, "LTC/ETH", "buy")
    res = econith.get_real_amount(trade, buy_order, order_obj)
...
def test_get_real_amount_multi(
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, "LTC/ETH", "buy")
    assert econith.get_real_amount(trade, buy_order_fee, order_obj) == expected_amount
    assert log_has(
        (
            "Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8, "
...
def test_get_real_amount_invalid_order(
    ...
    assert econith.get_real_amount(trade, limit_buy_order_usdt, order_obj) is None
...
def test_get_real_amount_fees_order(
    ...
    assert econith.get_real_amount(trade, market_buy_order_usdt_doublefee, order_obj) is None
...
def test_get_real_amount_wrong_amount(
    ...
    with pytest.raises(DependencyException, match=r"Half bought\? Amounts don't match"):
        econith.get_real_amount(trade, limit_buy_order_usdt, order_obj)
...
def test_get_real_amount_wrong_amount_rounding(
    ...
    assert pytest.approx(econith.get_real_amount(trade, limit_buy_order_usdt, order_obj)) == (
        amount * 0.001
    )
...
def test_get_real_amount_open_trade_usdt(default_conf_usdt, fee, mocker):
    ...
    assert econith.get_real_amount(trade, order, order_obj) is None
...
def test_get_real_amount_in_point(default_conf_usdt, buy_order_fee, fee, mocker, caplog):
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, "LTC/ETH", "buy")
    res = econith.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
    ...
    caplog.clear()
    econith.config["exchange"]["unknown_fee_rate"] = 1
    res = econith.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
...
def test_apply_fee_conditional(
    ...
    walletmock = mocker.patch("econith.wallets.Wallets.update")
    mocker.patch("econith.wallets.Wallets.get_free", return_value=wallet)
    trade = Trade(
        pair="LTC/ETH",
        amount=amount,
        exchange="binance",
    ...
    walletmock.reset_mock()
    # Amount is kept as is
    assert econith.apply_fee_conditional(trade, "LTC", amount, fee_abs, order) == amount_exp
    assert walletmock.call_count == 1
...
def test_apply_fee_conditional_multibuy(
    ...
    assert econith.apply_fee_conditional(trade, "LTC", amount, fee_abs, order1) == amount_exp
    assert walletmock.call_count == 1
    if fee_abs != 0 and amount_exp is None:
        assert log_has_re(r"Fee amount.*Eating.*dust\.", caplog)
...
def test_order_book_entry_pricing1(
    ...
    if exception_thrown:
        with pytest.raises(PricingError):
            econith.exchange.get_rate("ETH/USDT", side="entry", is_short=False, refresh=True)
        assert log_has_re(
...
def test_check_depth_of_market(default_conf_usdt, mocker, order_book_l2) -> None:
    ...
    assert econith._check_depth_of_market("ETH/BTC", conf, side=SignalDirection.LONG) is False
...
def test_startup_state(default_conf_usdt, mocker):
    ...
    assert worker.econith.state is State.RUNNING


def test_startup_trade_reinit(default_conf_usdt, mocker):
    mocker.patch(f"{EXMS}.exchange_has", MagicMock(return_value=True))
    reinit_mock = MagicMock()
    mocker.patch("econith.persistence.Trade.stoploss_reinitialization", reinit_mock)
...
def test_sync_wallet_dry_run(
    ...
    bot = get_patched_econithbot(mocker, default_conf_usdt)
...
def test_cancel_all_open_orders(
    ...
    buy_mock = mocker.patch("econith.econithbot.EconithBot.handle_cancel_enter")
    sell_mock = mocker.patch("econith.econithbot.EconithBot.handle_cancel_exit")

    econith = get_patched_econithbot(mocker, default_conf_usdt)
    create_mock_trades(fee, is_short=is_short)
    trades = Trade.session.scalars(select(Trade)).all()
    assert len(trades) == MOCK_TRADE_COUNT
    econith.cancel_all_open_orders()
...
def test_check_for_open_trades(mocker, default_conf_usdt, fee, is_short):
    ...
    econith.check_for_open_trades()
    assert econith.rpc.send_msg.call_count == 0
    ...
    econith.check_for_open_trades()
    assert econith.rpc.send_msg.call_count == 1
    assert "Handle these trades manually" in econith.rpc.send_msg.call_args[0][0]["status"]
...
def test_startup_update_open_orders(mocker, default_conf_usdt, fee, caplog, is_short):
    ...
    econith.startup_update_open_orders()
    assert not log_has_re(r"Error updating Order .*", caplog)
    caplog.clear()

    econith.config["dry_run"] = False
    econith.startup_update_open_orders()
    ...
    hto_mock = mocker.patch("econith.econithbot.EconithBot.handle_cancel_order")
...
def test_startup_backpopulate_precision(mocker, default_conf_usdt, fee, caplog):
    ...
    econith.startup_backpopulate_precision()
    trades = Trade.get_trades().all()
    for trade in trades:
...
def test_update_trades_without_assigned_fees(mocker, default_conf_usdt, fee, is_short):
    ...
    econith.update_trades_without_assigned_fees()
...
def test_reupdate_enter_order_fees(mocker, default_conf_usdt, fee, caplog, is_short):
    econith = get_patched_econithbot(mocker, default_conf_usdt)
    mock_uts = mocker.patch("econith.econithbot.EconithBot.update_trade_state")

    mocker.patch(f"{EXMS}.fetch_order_or_stoploss_order", return_value={"status": "open"})
    create_mock_trades(fee, is_short)
    trades = Trade.get_trades().all()

    econith.handle_insufficient_funds(trades[3])
    ...
    econith.handle_insufficient_funds(trade)
    # assert log_has_re(r"Trying to reupdate buy fees for .*", caplog)
...
def test_handle_insufficient_funds(mocker, default_conf_usdt, fee, is_short, caplog):
    ...
    econith.handle_insufficient_funds(trades[4])
...
def test_handle_onexchange_order(mocker, default_conf_usdt, limit_order, is_short, caplog):
    ...
    econith = get_patched_econithbot(mocker, default_conf_usdt)
    mock_uts = mocker.spy(econith, "update_trade_state")
    ...
    Trade.session.add(trade)
    econith.handle_onexchange_order(trade)
...
def test_handle_onexchange_order_changed_amount(
    ...
    econith.wallets = MagicMock()
    econith.wallets.get_owned = MagicMock(return_value=entry_order["amount"] * factor)
...
def test_handle_onexchange_order_fully_canceled_enter(
    ...
    trade.orders.append(Order.parse_from_ccxt_object(entry_order, "ADA/USDT", entry_side(is_short)))
    Trade.session.add(trade)
    assert econith.handle_onexchange_order(trade) is True
...
def test_get_valid_price(mocker, default_conf_usdt) -> None:
    ...
    patch_exchange(mocker)
    econith = EconithBot(default_conf_usdt)
    econith.config["custom_price_max_distance_ratio"] = 0.02
    ...
    valid_price_from_string = econith.get_valid_price(custom_price_string, proposed_price)
    valid_price_from_badstring = econith.get_valid_price(custom_price_badstring, proposed_price)
    valid_price_from_int = econith.get_valid_price(custom_price_int, proposed_price)
    valid_price_from_float = econith.get_valid_price(custom_price_float, proposed_price)

    valid_price_at_max_alwd = econith.get_valid_price(custom_price_over_max_alwd, proposed_price)
    valid_price_at_min_alwd = econith.get_valid_price(custom_price_under_min_alwd, proposed_price)
...
def test_update_funding_fees_schedule(
    ...
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch("econith.econithbot.EconithBot.update_funding_fees", return_value=True)
    default_conf["trading_mode"] = trading_mode
    default_conf["margin_mode"] = "isolated"
    econith = get_patched_econithbot(mocker, default_conf)

    time_machine.move_to(f"{t2} {tzoffset}", tick=False)
    # Check schedule jobs in debugging with econith._schedule.jobs
    econith._schedule.run_pending()

    assert econith.update_funding_fees.call_count == calls
...
def test_update_funding_fees(
    ...
    econith.execute_entry("ETH/USDT", 123, is_short=is_short)
    econith.execute_entry("LTC/USDT", 2.0, is_short=is_short)
    econith.execute_entry("XRP/USDT", 123, is_short=is_short)
...
def test_execute_entry_funding_fees_dca(mocker, default_conf_usdt, fee) -> None:
    ...
    assert econith.execute_entry(pair, stake_amount, trade=trade)
...
def test_position_adjust(mocker, default_conf_usdt, fee) -> None:
    ...
    assert econith.execute_trade_exit(
        trade=trade,
        limit=8,
...
def test_position_adjust2(mocker, default_conf_usdt, fee) -> None:
    ...
    assert econith.execute_entry(pair, amount)
    # Should create an closed trade with an no open order id
    # Order is filled and trade is open
    orders = Order.session.scalars(select(Order)).all()
    assert orders
...
def test_position_adjust3(mocker, default_conf_usdt, fee, data) -> None:
    ...
    for idx, (order, result) in enumerate(data):
        ...
        if order[0] == "buy":
            assert econith.execute_entry(pair, amount, trade=trade)
        else:
            assert econith.execute_trade_exit(
                trade=trade,
                limit=price,
...
def test_process_open_trade_positions_exception(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    mocker.patch(
        "econith.econithbot.EconithBot.check_and_call_adjust_trade_position",
        side_effect=DependencyException(),
    )

    create_mock_trades(fee)

    econith.process_open_trade_positions()
...
def test_check_and_call_adjust_trade_position(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    create_mock_trades(fee)
    caplog.set_level(logging.DEBUG)
    econith.strategy.adjust_trade_position = MagicMock(return_value=(10, "aaaa"))
    econith.process_open_trade_positions()
    assert log_has_re(r"Max adjustment entries for .* has been reached\.", caplog)
    assert econith.strategy.adjust_trade_position.call_count == 4

    caplog.clear()
    econith.strategy.adjust_trade_position = MagicMock(return_value=(-0.0005, "partial_exit_c"))
    econith.process_open_trade_positions()
