"""
Main ECONITH Quant worker class.
"""
...
from econith.configuration import Configuration
from econith.constants import PROCESS_THROTTLE_SECS, RETRY_TIMEOUT, Config
from econith.enums import RPCMessageType, State
from econith.exceptions import OperationalException, TemporaryError
from econith.exchange import timeframe_to_next_date
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.econith = EconithBot(self._config)
    ...
    def _worker(self, old_state: State | None) -> State:
        ...
        """
        state = self.econith.state

        # Log state transition
        if state != old_state:
            if old_state != State.RELOAD_CONFIG:
                self.econith.notify_status(f"{state.name.lower()}")
            ...
            if state in (State.RUNNING, State.PAUSED) and old_state not in (
                ...
                self.econith.startup()

            if state == State.STOPPED:
                self.econith.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.econith.strategy.version()
    ...
    def _process_stopped(self) -> None:
        self.econith.process_stopped()

    def _process_running(self) -> None:
        try:
            self.econith.process()
            ...
            self.econith.notify_status(
                f"*OperationalException:*\n```\n{tb}```\n {hint}", msg_type=RPCMessageType.EXCEPTION
            )

            logger.exception("OperationalException. Stopping trader ...")
            self.econith.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current econithbot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current econith modules
        self.econith.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.econith.notify_status(f"{State(self.econith.state)} after config reloaded")
    ...
    def exit(self) -> None:
        ...
        if self.econith:
            self.econith.notify_status("process died")
            self.econith.cleanup()
