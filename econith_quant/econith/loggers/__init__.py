...
from econith.loggers.buffering_handler import FTBufferingHandler
from econith.loggers.ft_rich_handler import FtRichHandler
from econith.loggers.rich_console import get_rich_console


# from econith.loggers.std_err_stream_handler import FTStdErrStreamHandler
...
def _set_log_levels(
    ...
    third_party_loggers = {
        "econith": logging.INFO if verbosity < 1 else logging.DEBUG,
        "econith.exchange.exchange_ws": logging.INFO if verbosity <= 1 else logging.DEBUG,
...
def _create_log_config(config: Config) -> dict[str, Any]:
    ...
    log_config = deepcopy(config.get("log_config", ECONITH_LOGGING_CONFIG))
    ...
    for handler_config in log_config.get("handlers", {}).values():
        if handler_config.get("class") == "econith.loggers.ft_rich_handler.FtRichHandler":
            ...
            try:
                ...
                raise OperationalException(
                    ...
                    "directories. If you're running econith using docker, you see this error "
