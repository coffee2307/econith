...
from econith.enums import RunMode, State
from econith.exceptions import OperationalException
from econith.rpc import RPC
from econith.rpc.api_server.api_pairlists import handleExchangePayload
...
from econith.rpc.api_server.deps import (
    get_config,
    get_exchange,
    get_rpc,
    get_rpc_optional,
...
def show_config(rpc: RPC | None = Depends(get_rpc_optional), config=Depends(get_config)):
    ...
    if rpc:
        state = rpc._econith.state
        strategy_version = rpc._econith.strategy.version()
...
def markets(
    ...
    if not rpc or config["runmode"] == RunMode.WEBSERVER:
        ...
        exchange = rpc._econith.exchange
...
def get_strategy(
    ...
    if not rpc or config["runmode"] == RunMode.WEBSERVER:
        ...
        # trade mode
        strategy_obj = rpc._econith.strategy
