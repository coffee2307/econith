...
def chown_user_directory(directory: Path) -> None:
    ...
    if running_in_docker():
        try:
            ...
            subprocess.check_output(["sudo", "chown", "-R", "econithuser:", str(directory.resolve())])
        except Exception:
    ...
    if not folder.is_dir():
        if create_dir:
            ...
            raise OperationalException(
                ...
                "Please use `econith create-userdir` to create a user directory"
            )
        ...
        if not subfolder.is_dir():
            if subfolder.exists() or subfolder.is_symlink():
                raise OperationalException(
                    ...
                    "ECONITH Quant requires this to be a directory."
                )
