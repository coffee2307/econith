...
from econith.data.history import get_timerange, load_data
...
from econith.exchange import timeframe_to_prev_date, timeframe_to_seconds
from econith.misc import pair_to_filename
...
def generate_profit_graph(
    ...
    fig["layout"].update(title="ECONITH Quant Profit plot")
    fig["layout"]["yaxis1"].update(title="Price")
...
def generate_plot_filename(pair: str, timeframe: str) -> str:
    ...
    file_name = "econith-plot-" + pair_s + "-" + timeframe + ".html"
...
def plot_profit(config: Config) -> None:
    ...
    store_plot_file(
        fig,
        filename="econith-profit-plot.html",
        directory=config["user_data_dir"] / "plot",
