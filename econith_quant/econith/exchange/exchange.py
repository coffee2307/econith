...
from econith.configuration import remove_exchange_credentials
from econith.constants import (
...
from econith.data.converter import (
...
from econith.exchange.exchange_types import (
    CcxtBalances,
    CcxtOrder,
    CcxtPosition,
    FtHas,
...
from econith.exchange.exchange_ws import ExchangeWS
...
from econith.util import FtTTLCache, PeriodicCache, dt_from_ts, dt_now
from econith.util.datetime_helpers import dt_humanize_delta, dt_ts, format_ms_time
...
class Exchange:
    ...
    def market_is_tradable(self, market: dict[str, Any]) -> bool:
        """
        Check if the market symbol is tradable by ECONITH Quant.
    ...
    def validate_timeframes(self, timeframe: str | None) -> None:
        ...
        if (
            ...
            raise ConfigurationError("Timeframes < 1m are currently not supported by ECONITH Quant.")
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        """
        Checks if econith can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by econith on this exchange
        """
        ...
        if allow_none_margin_mode and margin_mode is None:
            ...
            if not any(
                ...
                raise ConfigurationError(
                    f"ECONITH Quant does not support '{trading_mode}' on {self.name}."
                )
        ...
        if not allow_none_margin_mode and (
            ...
            raise ConfigurationError(
                f"ECONITH Quant does not support '{mm_value}' '{trading_mode}' on {self.name}."
            )
    ...
    def fetch_dry_run_order(self, order_id: str) -> CcxtOrder:
        ...
        try:
            ...
            from econith.persistence import Order
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            if market["inverse"]:
                raise OperationalException("ECONITH Quant does not yet support inverse contracts")
            ...
            raise OperationalException(
                "ECONITH Quant only supports isolated futures for leverage trading"
            )
