...
from econith.configuration import TimeRange, validate_config_consistency
from econith.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
...
from econith.data.converter import trim_dataframe, trim_dataframes
from econith.data.dataprovider import DataProvider
from econith.data.metrics import combined_dataframes_with_rel_mean
...
from econith.exchange.exchange import TICK_SIZE, Exchange
from econith.ft_types import (
    BacktestContentType,
...
from econith.optimize.backtest_caching import get_strategy_run_id
...
from econith.persistence import (
    CustomDataWrapper,
    LocalTrade,
    Order,
    PairLocks,
...
from econith.util import FtPrecise, dt_now, get_progress_tracker
from econith.util.migrations import migrate_data
...
class Backtesting:
    ...
    def __init__(
        ...
        *,
        ...
        if self.config.get("freqai_backtest_live_models", False):
            from econith.freqai.utils import get_timerange_backtest_live_models
    ...
    def _get_close_rate_for_roi(
        ...
        if roi is not None and roi_entry is not None:
            ...
            if trade_dur == 0 and (
                ...
                # details: https: // github.com/econith/econith/issues/6261
        ...
        if exit_.exit_flag:
            ...
            if exit_.exit_type in (
                ...
                if order_type == "limit":
                    ...
                    # econith does not support this in live, and the order would fill immediately
                    if trade.is_short:
    ...
    def get_valid_entry_price_and_stake(
        ...
        if order_type == "limit":
            ...
            # which econith does not support in live.
